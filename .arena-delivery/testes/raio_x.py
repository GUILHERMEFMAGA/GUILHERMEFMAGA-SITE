...
def checar_evolucao():
    caminho = RAIZ / "agente" / "evolucao.py"
    if not caminho.exists():
        problema("evolucao", "agente/evolucao.py nao existe — as 75 ideias ficaram sem corpo",
                 "entregue o modulo de evolucao governada junto com o portao proprio")
        return
    try:
        texto = caminho.read_text(encoding="utf-8")
        arvore = ast.parse(texto)
    except (SyntaxError, OSError) as erro:
        problema("evolucao", "modulo de evolucao nao compila: %s" % erro,
                 "cole o arquivo agente/evolucao.py inteiro e rode o raio-x de novo")
        return
    obrigatorias = {"validar_skill", "registrar_tombstone", "registrar_claim",
                    "simular_plano", "red_team_check", "diff_semantico",
                    "auditar_ambiente", "plano_noturno_seguro"}
    funcoes = {no.name for no in ast.walk(arvore) if isinstance(no, ast.FunctionDef)}
    faltas = sorted(obrigatorias - funcoes)
    ideias = None
    for no in arvore.body:
        if isinstance(no, ast.Assign) and any(isinstance(alvo, ast.Name) and alvo.id == "IDEIAS" for alvo in no.targets):
            try:
                ideias = ast.literal_eval(no.value)
            except (ValueError, SyntaxError):
                ideias = None
    ids = [item.get("id") for item in ideias] if isinstance(ideias, list) and all(isinstance(item, dict) for item in ideias) else []
    matriz_ok = len(ids) == 75 and len(set(ids)) == 75 and set(ids) == set(range(1, 76))
    try:
        config_evolucao = ler_json_seguro(RAIZ / "fluxos" / "regras.json").get("evolucao", {})
    except (OSError, json.JSONDecodeError):
        config_evolucao = {}
    contrato_ok = (config_evolucao.get("ligado") is True
                   and config_evolucao.get("cron_somente_audita") is True
                   and config_evolucao.get("promover_so_com_humano") is True
                   and config_evolucao.get("permitir_codigo_runtime") is False
                   and config_evolucao.get("ideias_catalogadas") == 75)
    if faltas:
        problema("evolucao", "faltam primitivas: %s" % ", ".join(faltas),
                 "o modulo precisa manter skills, memoria, seguranca, avaliacao e diff")
    elif not matriz_ok:
        problema("evolucao", "matriz IDEIAS nao cobre exatamente 1..75", "nao esconda ideia em comentario: mantenha a matriz auditavel")
    elif not contrato_ok:
        problema("evolucao", "bloco evolucao do cerebro permite autonomia insegura ou esta incompleto",
                 "mantenha cron_somente_audita=true, promover_so_com_humano=true e permitir_codigo_runtime=false")
    else:
        ok("evolucao", "75 ideias catalogadas; skills declarativas, memoria, red-team, diff e auditoria presentes")
...
def main():
    ...
    try:
        ...
        problema("evolucao", "a checagem quebrou por dentro: %s" % erro, "manda o print que eu conserto o raio-x")
    print("-" * 78)
