...
AJUDA = False
TRAVA_VELHA = False
FLAGS_CONHECIDAS = {"--so-olhar", "--vigiar", "--ensaiar", "--evoluir", "--podar-sombra", "--ajuda"}
...
def _flags(argv, trava_velha=False):
    ...
    global SO_OLHAR, VIGIAR, ENSAIAR, EVOLUIR, PODAR, AJUDA, TRAVA_VELHA
...
def ajuda_texto():
    return ("super-agente — usos:\n"
            ...
            "  python agente\\loop.py --evoluir        :: audita as 75 ideias sem autoalterar o codigo\n"
...
def auditar_evolucao():
    """Audita as ideias e prepara tarefas seguras, sem escrever codigo.

    O cron pode chamar esta função durante a madrugada. Ela só lê o ambiente e
    imprime um plano; promoção, alteração de regra e execução continuam humanas.
    """
    import importlib.util
    caminho = Path(__file__).with_name("evolucao.py")
    spec = importlib.util.spec_from_file_location("super_agente_evolucao", caminho)
    if spec is None or spec.loader is None:
        return ["laboratorio de evolucao ausente: nenhuma auto-mudanca foi feita"]
    modulo = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(modulo)
    auditoria = modulo.auditar_ambiente(RAIZ)
    matriz = auditoria["matrix"]
    linhas = ["evolucao governada %s: %d ideias catalogadas, matriz_ok=%s" %
              (auditoria["version"], matriz["total"], matriz["matriz"]["ok"]),
              "evolucao: %d skills declarativas, codigo runtime=%s, git pelo agente=%s" %
              (auditoria["skills"], auditoria["constitution"]["self_written_code"],
               auditoria["constitution"]["git_by_agent"])]
    for tarefa in modulo.plano_noturno_seguro():
        linhas.append("evolucao plano: %s -> %s" % (tarefa["id"], tarefa["acao"]))
    return linhas


def rodar():
        if EVOLUIR and not VIGIAR:
            for linha in auditar_evolucao():
                print(linha)
            print("evolucao: nenhuma regra, skill ou codigo foi promovido automaticamente")
            return 0
        if VIGIAR and (SO_OLHAR or ENSAIAR):
            ...
            if EVOLUIR:
                for linha in auditar_evolucao():
                    print(" -", linha)
                print(" - evolucao: cron somente prepara auditoria; nenhuma promoção automatica")
            return 0
