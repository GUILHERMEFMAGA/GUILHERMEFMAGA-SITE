...
## ATUALIZACAO DESTA ENTREGA — EVO governada
O espelho desta sessão contém `agente/evolucao.py` e `testes/testar_evolucao.py`.
As 75 ideias estão catalogadas e cobertas por primitivas locais; as ideias que
exigem LLM/Transformer ficam explicitamente marcadas como incompatíveis, não são
simuladas por marketing. `--evoluir` só audita e prepara plano; não escreve Python,
não executa código gerado, não roda Git e não promove regra. Portão de evolução,
portão legado e raio-x foram executados verdes antes desta entrega.
