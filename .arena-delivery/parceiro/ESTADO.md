...
## 22/09 noite — EVO: 75 ideias catalogadas e primitivas governadas no espelho
- Pedido do Guilherme: colocar no codigo as 75 ideias, depois da auditoria completa para nao duplicar.
- Leitura/auditoria antes da edicao: 19 Python + 9 JSON/JSONL parseados, 0 erros; 205 funcoes auditadas, 0 corpos identicos. O NUCLEO `agente-ia/` e o super-agente `.arena-delivery/` continuam separados; nenhuma fusao cega foi feita.
- Criado `agente/evolucao.py`: matriz auditavel 1..75; skills declarativas sem Python runtime; curriculo por fraqueza; maioria/best-of-N/beam/canary; world model; recompensa por etapa; decaimento/compressao; traces; tombstones encadeados; claim ledger; contradicoes; Bayes/Kalman; solver AST; verificacao de artefato; property/fuzz/chaos; red-team/honeytokens; capability passports; saida zero-trust; snapshots/diff/regressao; cache semantico; calibracao e custo local.
- Contrato de seguranca: cron so audita; codigo runtime e rejeitado; skill exige testes + aprovacao humana; Git nunca roda pelo agente. `--evoluir` imprime plano, nao promove nada.
- `tick.bat` e madrugada passaram a chamar `--evoluir` em modo observacao; `verificar-tudo.bat` agora roda portao de evolucao, raio-x e portao legado. `.gitignore` protege ledgers JSONL.
- Portao de evolucao: ABERTO. Portao legado: ABERTO 17/17. Raio: 21 ok | 9 dicas | 1 aviso | 0 problemas. Nenhum push foi feito; espelho continua apenas no View All Changes desta sessao.
