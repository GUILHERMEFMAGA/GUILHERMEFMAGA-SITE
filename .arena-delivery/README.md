...
python agente\loop.py --evoluir            :: audita as 75 ideias sem escrever codigo nem promover regra
...
## 🧠 Evolução governada — as 75 ideias no código

O arquivo `agente/evolucao.py` concentra as ideias em primitivas locais e
verificáveis. Ele possui matriz explícita das 75 ideias, biblioteca de skills
**declarativas**, currículo por fraqueza, votação, beam search, recompensas por
etapa, canary probes, simulação em mundo virtual, memória com decaimento,
compressão por saliência, traces operacionais, tombstones encadeados, claim
ledger, contradição, Bayes/Kalman, solver aritmético seguro, verificação de
artefato, property checks, fuzz/chaos cases, red-team, honeytokens,
capability passports, validação zero-trust, diff semântico, snapshots,
regressão, cache semântico e calibração.

A fronteira não é negociável: o agente não escreve Python executável em runtime,
não executa código gerado, não roda Git e não promove alteração sozinho. A skill
salva no disco é um manifesto com os quatro verbos da coleira; `approved_by_human`
e `tests_passed` são obrigatórios para reutilização. `--evoluir` e o tick noturno
apenas auditam, calculam e imprimem um plano seguro. O botão humano continua
sendo a promoção.

Para provar essa camada isoladamente:

```
python testes\\testar_evolucao.py
python agente\\loop.py --evoluir
```
