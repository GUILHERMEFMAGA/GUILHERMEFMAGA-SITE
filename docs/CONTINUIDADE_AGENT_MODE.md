...
Atualizado em 14/09/2026 (cobre **r21 → r87**; releases posteriores a esta data: veja `git log` e os comentários do PR #4). Este documento descreve a **r21** (confiabilidade das respostas
...
- Releases publicados: **r21 → r87** (o número r77 não foi usado — o STT saiu como r79, na ordem de publicação). Suíte: **961 testes OK** (skipped=3). Commits de referência: `255faab` (r73), `a8ba70b` (lista-100), `e8072fb` (auditoria completa); r87 = ponta do log da branch da sessão atual (`arena/01a09bca-guilhermefmaga-site` — a branch de ENTREGA do atualizador).
- Código: `agente.py` ~39,7 mil linhas (513 ferramentas @tool + funções r74 de ritmo/céu + r75 de casa/fila/cron/sandbox/cofre/convidado + r76 de cerebro/auditoria/visao/manual + r78 de automação c/ Agendador do Windows + r79 de voz/STT local + r80 de resposta organizada + r81 de entrega certa/escudo de entrada + r82 de caixa do dono + r83 de diagnostico de microfone + r84 de rota robusta + r85 de caule-diagnost/atualizar-que-fala/mensagem-certa + r86 de whatsapp-de-verdade/numero-direto/salvar-sem-nome + r87 de memoria-e-auditoria-whatsapp), `main.py`, `iniciar.bat` (CRLF puro), `requirements.txt` (faixas major: langchain-openai>=1.0,<2.0; langchain-google-genai>=4.0,<5.0).
...
- **r83 — DIAGNÓSTICO DE MICROFONE (do bug real do STT no PC do dono, 14/09)**:
  ...
  `-r83`. Proxima prova no PC: 'atualizar agora' (p/ r87) +
