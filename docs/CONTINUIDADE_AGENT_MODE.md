...
- **r92 — CEGO NÃO MEXE NA TELA (log real do PC, 14/09, boot r91)**: no PC
  ...
  testes OK** — a suíte PASSOU DE 1000. Selo `-r92`. Próxima prova no
  PC: 'atualizar agora' (p/ r92; 'Baixei a versao nova' antes de
  reiniciar) + 'interaja com NOME' (deve sair o diagnóstico do leitor no
  terminal e o WhatsApp NÃO deve pular a frente) + ligar o leitor (visão
  local r76 ou GEMINI_API_KEY) + o modo conversa de verdade.
