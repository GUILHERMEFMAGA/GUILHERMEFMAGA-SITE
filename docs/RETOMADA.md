...
- **Última release publicada: r75** — "A CASA" (6 melhorias da LISTA-IMPOSSIVEL: 28 fila de fundo `fila: <tarefa>`, 29 cron local 2.0 com dias da semana `cron adicionar <nome> as HH:MM: <tarefa>`, 30 diário de lentidão com causa provável, 39 sandbox de código com limites + 'sim' sempre, 40 cofre de chaves com DPAPI (o chaves.txt fica INTACTO como backup), 41 modo convidado de sessão; kill-switches `fila_de_fundo`, `agenda_cron`, `diario_lentidao`, `sandbox_codigo`, `cofre_chaves`, `modo_convidado`). Antes: r74 "RITMO E CÉU" (dry-run), r73 "BOAS-VINDAS DE VOLTA" (`sessao.json`), r72 honestidade, r71 cérebro persistente.
- **Suíte: 817 testes OK** (skipped=3). `agente.py`: ~38,7 mil linhas, 513 ferramentas @tool, 24+6+7 rotas de comando. Selo atual: `[Motor e avaliacao local 2026-09-11-r75]`.
...
1. **O PC do dono está na r71** — ele ainda vai rodar `atualizar agora` pra pular de uma vez pra r75 (r72+r73+r74+r75); cobrar prints da abertura com o BOAS-VINDAS DE VOLTA.
2. **Leva autorizada pelo dono: 19 itens da LISTA-IMPOSSIVEL** (`esqueletos_ideias/2026-09-13-lista-impossivel-ia-local.md`) — **r74 publicada (31–37)**; **r75 publicada (28, 29, 30, 39, 40, 41)**; **r76** = 14 exportar/importar cérebro, 15 diff do cérebro, 42 auditoria estendida, 43 visão local, 44 manual do usuário; **r77** = 38 STT local (obra grande, release à parte).
...
```
Esperado: `Ran 817 tests ... OK (skipped=3)` e o selo `-r75` em agente.py. Deu certo? Você está EXATAMENTE onde paramos.
...
- `docs/CONTINUIDADE_AGENT_MODE.md` — diário completo r1→r75 + regras + lições + posicionamento honesto.
- `esqueletos_ideias/` — listas (20/30/50+40/80/100/impossível = SÓ PROPOSTA), análise de lacunas, auditoria.
- `tests/` — 84 arquivos, 817 testes (loader em test_roteamento_conversa.py).
