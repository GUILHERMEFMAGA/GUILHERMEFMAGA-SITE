...
- **Última release publicada: r78** — "AUTOMAÇÃO PROFISSIONAL" (o que o dono pediu após a auditoria de honestidade): `agendar windows <nome> as HH:MM: <comando>` (dias opcionais) cria tarefas no AGENDADOR DE TAREFAS DO WINDOWS (schtasks) — a automação roda MESMO COM O AGENTE FECHADO (o Windows garante a hora); `agendar windows` lista, `agendar windows apagar <nome>` remove; sempre com 'sim'; os arquivos gerados ficam em `_tarefas_windows/` (.bat CRLF puro, sem if/REM); no modo tarefa o agente abre sem falar (env `AGENTE_TAREFA_WINDOWS`); kill-switch `tarefas_windows`. **Correção real incluída:** o parse da rota `cron adicionar` da r75 quebrava a hora em dois (split no ':') — corrigido (regex fullmatch). Antes: r76 "CONHECIMENTO E CASA AMPLIADA", r75 "A CASA", r74 "RITMO E CÉU" (dry-run), r73 "BOAS-VINDAS DE VOLTA" (`sessao.json`), r72 honestidade, r71 cérebro persistente.
- **Suíte: 855 testes OK** (skipped=3). `agente.py`: ~39,5 mil linhas, 513 ferramentas @tool, 24+6+7+6+1 rotas de comando. Selo atual: `[Motor e avaliacao local 2026-09-11-r78]` (r77 = STT, reservada).
...
1. **O PC do dono está na r71** — ele ainda vai rodar `atualizar agora` pra pular de uma vez pra r78 (r72+r73+r74+r75+r76+r78); cobrar prints da abertura com o BOAS-VINDAS DE VOLTA.
2. **Leva autorizada pelo dono: 19 itens da LISTA-IMPOSSIVEL** (`esqueletos_ideias/2026-09-13-lista-impossivel-ia-local.md`) — **r74 publicada (31–37)**; **r75 publicada (28, 29, 30, 39, 40, 41)**; **r76 publicada (14, 15, 42, 43, 44)**; **r78 publicada (automação profissional c/ Agendador do Windows — a pedido do dono, pulando o número r77)**; **r77** = 38 STT local (obra grande, release à parte — download do whisper.cpp no PC do dono exigirá 'sim' explícito).
...
```
Esperado: `Ran 855 tests ... OK (skipped=3)` e o selo `-r78` em agente.py. Deu certo? Você está EXATAMENTE onde paramos.
...
- `docs/CONTINUIDADE_AGENT_MODE.md` — diário completo r1→r78 + regras + lições + posicionamento honesto.
- `esqueletos_ideias/` — listas (20/30/50+40/80/100/impossível = SÓ PROPOSTA), análise de lacunas, auditoria.
- `tests/` — 86 arquivos, 855 testes (loader em test_roteamento_conversa.py).
