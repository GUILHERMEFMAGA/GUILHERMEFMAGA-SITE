...
- **Última release publicada: r76** — "CONHECIMENTO E CASA AMPLIADA" (5 melhorias da LISTA-IMPOSSIVEL: 14 `exportar cerebro`/`importar cerebro: <arquivo>` — backup versionado do cérebro, import MESCLA (nunca apaga), 15 `diff cerebro` — "o que aprendi esta semana?" (fatos novos ganham data `quando`), 42 `auditoria completa` — motor+rotas+gatilhos+cérebro num relatório único .md, 43 `ver <imagem>` — visão local com modelo GGUF SEPARADO (o principal segue intacto; nunca baixa sem 'sim'), 44 `manual` — manual do usuário gerado do próprio código; kill-switches `exportar_cerebro`, `diff_cerebro`, `auditoria_completa`, `visao_local`, `manual_usuario`). Antes: r75 "A CASA", r74 "RITMO E CÉU" (dry-run), r73 "BOAS-VINDAS DE VOLTA" (`sessao.json`), r72 honestidade, r71 cérebro persistente.
- **Suíte: 839 testes OK** (skipped=3). `agente.py`: ~39,2 mil linhas, 513 ferramentas @tool, 24+6+7+6 rotas de comando. Selo atual: `[Motor e avaliacao local 2026-09-11-r76]`.
...
1. **O PC do dono está na r71** — ele ainda vai rodar `atualizar agora` pra pular de uma vez pra r76 (r72+r73+r74+r75+r76); cobrar prints da abertura com o BOAS-VINDAS DE VOLTA.
2. **Leva autorizada pelo dono: 19 itens da LISTA-IMPOSSIVEL** (`esqueletos_ideias/2026-09-13-lista-impossivel-ia-local.md`) — **r74 publicada (31–37)**; **r75 publicada (28, 29, 30, 39, 40, 41)**; **r76 publicada (14, 15, 42, 43, 44)**; **r77** = 38 STT local (obra grande, release à parte — download do whisper.cpp no PC do dono exigirá 'sim' explícito).
...
```
Esperado: `Ran 839 tests ... OK (skipped=3)` e o selo `-r76` em agente.py. Deu certo? Você está EXATAMENTE onde paramos.
...
- `docs/CONTINUIDADE_AGENT_MODE.md` — diário completo r1→r76 + regras + lições + posicionamento honesto.
- `esqueletos_ideias/` — listas (20/30/50+40/80/100/impossível = SÓ PROPOSTA), análise de lacunas, auditoria.
- `tests/` — 85 arquivos, 839 testes (loader em test_roteamento_conversa.py).
