...
1. **O PC do dono está na r73** (prova no log de 14/09: selo r73 e `atualizar agora` dizendo "conteudo identico, nada foi trocado" — porque as URLs antigas apontavam para a branch congelada na r73). Para pular para a r81, ele precisa rodar UMA VEZ SÓ o passo de PowerShell pronto (comando único de copiar/colar — entregue ao dono em 14/09); a partir da r81 o `atualizar agora` volta a funcionar para sempre. Depois: cobrar prints da abertura + provar `tabela`/`grafico`/`ideias` (r80) e `baixar stt`/`falar` (r79).
...
```
Esperado: `Ran 903 tests ... OK (skipped=3)` e o selo `-r82` em agente.py. Deu certo? Você está EXATAMENTE onde paramos.
...
- `docs/CONTINUIDADE_AGENT_MODE.md` — diário completo r1→r82 + regras + lições + posicionamento honesto.
- `esqueletos_ideias/` — listas (20/30/50+40/80/100/impossível = SÓ PROPOSTA), análise de lacunas, auditoria.
- `tests/` — 90 arquivos, 903 testes (loader em test_roteamento_conversa.py).
