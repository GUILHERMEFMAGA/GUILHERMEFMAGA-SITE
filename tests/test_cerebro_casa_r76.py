...
_SELO = "[Motor e avaliacao local 2026-09-11-r83]"
...
class TestCerebroCasaR76(unittest.TestCase):
    ...
    def test_selo_r76(self):
        with open(AGENTE_PY, encoding='utf-8') as f:
            fonte = f.read()
        self.assertEqual(fonte.count('[Motor e avaliacao local 2026-09-11-r83]'), 1)
    ...
    def test_exportar_grava_backup_versionado(self):
        ...
        self.assertEqual(pacote['selo'], '2026-09-11-r83')  # lido do proprio codigo (fake agora selado r79)
    ...
    def test_auditoria_gera_relatorio_unico(self):
        ...
        for secao in ('## 1. Motor local', '## 2. Rotas e ferramentas', '## 3. Gatilhos',
                      ...
                      'Selo do codigo: 2026-09-11-r83', 'C:/llama-server.exe', 'C:/modelo.gguf',
    ...
    def test_manual_gerado_do_codigo(self):
        ...
        with open(caminho, encoding='utf-8') as f:
            txt = f.read()
        for trecho in ('MANUAL DO AGENTE', 'Versao do codigo: 2026-09-11-r83',
